...
Version 0.1.4 is the certified survey-binary release candidate. Its expanded
model adapters, pweights, linear ancillary parameter, sample alignment,
cluster-robust covariance, linear-panel, random-intercept Gaussian ML,
bivariate-probit, IV-probit, GEE, multiple-imputation, and narrow one-stage
survey routes are covered by focused unit tests. The established families are
also covered by the full numerical acceptance suite. Cross-language benchmarks
live in `tools/stata-benchmarks/`.

Post-0.1.4 development prioritizes nonlinear panel families first and general
GLMM/multilevel models second. See
