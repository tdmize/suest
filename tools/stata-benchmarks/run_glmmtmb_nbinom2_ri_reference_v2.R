...
fit_glmm_nbinom2 <- function(formula, data = dat) {
  model <- glmmTMB::glmmTMB(formula, data = data, family = glmmTMB::nbinom2("log"))
  suest:::.suest_model_adapter(model)
  # The positive benchmark deliberately requires well-estimated interior variance.
  stopifnot(exp(2*model$fit$par["theta"]) > .01,
    sqrt(model$sdr$cov.fixed["theta", "theta"]) < 1)
  model
}
...
extract_model <- function(model) {
  ...
  list(coefficients = parameters, vcov = covariance,
    ...
    random_variance = unname(sigma2),
    log_sd_se = sqrt(model$sdr$cov.fixed["theta", "theta"]),
...
left_data <- dat[!is.na(dat$y1_left), ]
right_data <- dat[!is.na(dat$y2_right), ]
...
reference <- list(
  metadata = list(
    benchmark_revision = 2L, disjoint_groups = "odd versus even id",
...
saveRDS(reference, "glmmtmb_nbinom2_ri_v2_r_reference.rds")
