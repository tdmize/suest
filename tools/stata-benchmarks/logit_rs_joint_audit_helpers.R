...
audit_logit_rs_joint <- function(s, r, d, outcomes, cluster, components, evaluate_at = c("Stata", "R")) {
  ...
  group_ll <- function(b,y) {
    ...
    vapply(groups,function(idx) {
      ...
      u <- c(0,0)
      for(k in 1:100) {
        p <- plogis(ei+drop(zi%*%u)); g <- drop(crossprod(zi,p-yi)+P%*%u)
        if(max(abs(g)) < 1e-10) break
        H <- crossprod(zi,zi*as.numeric(p*(1-p)))+P; step <- drop(solve(H,g)); scale <- 1
        while(objective(u-scale*step) > objective(u)+1e-12 && scale>1e-8) scale <- scale/2
        u <- u-scale*step
      }
      p <- plogis(ei+drop(zi%*%u)); g <- drop(crossprod(zi,p-yi)+P%*%u)
      stopifnot(max(abs(g))<1e-8)
      H <- crossprod(zi,zi*as.numeric(p*(1-p)))+P
      -objective(u)-logdet/2-as.numeric(determinant(H,logarithm=TRUE)$modulus)/2
    },numeric(1))
  }
  ...
  results <- lapply(1:2, function(i) {
    b <- evaluation_parameters[(i-1L)*6L+1:6]; y <- d[[outcomes[i]]]
    ...
    # Sensitivity: natural (beta, A, B, C) information transformed to these
    # log-SD/Fisher coordinates, retaining total-score chain-rule terms.
    ...
    # Native t=sinh(z): delta-transformed inverse information retains g_z*rho.
    Tnative <- matrix(0, 6, 6); Tnative[6, 6] <- g[6]*rho
  ...
  Bt <- matrix(0, 12, 12)
  Bt[1:6, 1:6] <- results[[1]]$bread_native_delta
  Bt[7:12, 7:12] <- results[[2]]$bread_native_delta
  ...
  Bc <- matrix(0, 12, 12)
  Bc[1:6, 1:6] <- components[[outcomes[1]]]$vcov
  Bc[7:12, 7:12] <- components[[outcomes[2]]]$vcov
