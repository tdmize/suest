...
dat <- read.csv("suest_r_panel_logit_re_crosslang_benchmark.csv")
fit_re_logit <- function(formula) {
  start <- c(coef(glm(formula, data = dat, family = binomial("logit"))), sigma = 1)
  pglm::pglm(
...
y1 <- fit_re_logit(y1 ~ x + z)
y2 <- fit_re_logit(y2 ~ x + z)
balanced <- suest(
...
y1_partial <- fit_re_logit(y1_partial ~ x + z)
y2_partial <- fit_re_logit(y2_partial ~ x + z)
partial <- suest(
  y1_partial, y2_partial, model_names = c("Y1", "Y2"),
  observation_id = c("id", "time"), cluster = "higher"
)

y1_left <- fit_re_logit(y1_left ~ x + z)
y2_right <- fit_re_logit(y2_right ~ x + z)
disjoint <- suest(
...
saveRDS(reference, "panel_logit_re_r_reference.rds")

cat("PANEL_LOGIT_RE_R_REFERENCE_COMPLETE=1\n")
