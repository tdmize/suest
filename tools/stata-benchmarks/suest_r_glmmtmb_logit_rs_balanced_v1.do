...
log using suest_r_glmmtmb_logit_rs_balanced_v1.log, replace text name(logitrs)
di as result "BENCHMARK_FILE_REVISION=1"
di as txt "LOGIT RANDOM SLOPES: TWO BALANCED COMPONENTS AND ONE JOINT FIT ONLY"
...
do suest_r_glmmtmb_logit_rs_start_values.do
matrix logitrs_components = J(2, 6, .)
matrix rownames logitrs_components = y1 y2
...
foreach outcome in y1 y2 {
    ...
    if e(converged) != 1 | e(N) != 1200 | missing(e(ll)) | abs(e(ll)-scalar(logitrs_r_`outcome'_ll)) > 1e-4 | scalar(logitrs_ok) != 1 {
...
local model "gsem (y1 <- x z I1[id]@1 c.x#S1[id]@1, family(bernoulli) link(logit)) (y2 <- x z I2[id]@1 c.x#S2[id]@1, family(bernoulli) link(logit)), cov(I1[id]*S1[id] I2[id]*S2[id] I1[id]*I2[id]@0 I1[id]*S2[id]@0 S1[id]*I2[id]@0 S1[id]*S2[id]@0) intmethod(laplace) adaptopts(tolerance(1e-12)) vce(robust)"
...
di as result "LOGIT_RS_BALANCED_COMPLETE=1"
