...
audit_poisson_rs_joint <- function(s, r, d, outcomes, cluster, components) {
  ...
  results <- lapply(1:2, function(i) {
    b <- s$coefficients[(i-1L)*6L+1:6]; y <- d[[outcomes[i]]]
  ...
  stopifnot(abs(ll_difference) < 1e-5,
    ...
    comparison["component_bread_stata_meat_vs_R", "max_abs_covariance"] < 1e-6,
    comparison["component_bread_stata_meat_vs_R", "max_relative_diagonal_sd"] < 1e-4)
  list(components = results, comparison = comparison, independent_bread = B,
    ...
    metadata = list(outcomes = outcomes, cluster = cluster, n_clusters = s$n_clusters,
