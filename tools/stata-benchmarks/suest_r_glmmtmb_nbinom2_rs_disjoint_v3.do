...
log using suest_r_glmmtmb_nbinom2_rs_disjoint_v3.log, replace text name(nb2rs)
di as result "BENCHMARK_FILE_REVISION=3"
di as txt "NB2 RANDOM SLOPES: DISJOINT ODD/EVEN GROUPS, 100 UNION CLUSTERS; THREE FITS ONLY"
di as txt "R seeds are checked at zero iterations; full mixed models are capped at 15 iterations."
di as txt "Fixed-effects starting fits are capped at 15 iterations. Stop at the first failed check."
...
do suest_r_glmmtmb_nbinom2_rs_disjoint_starts.do
matrix nb2rs_components = J(2, 7, .)
matrix rownames nb2rs_components = y1_left y2_right
...
void nb2rs_component_check() {
    ...
    st_numscalar("nb2rs_ok", rows(v) == 7 & cols(v) == 7 & !hasmissing(v) & ng[1,1] == 50)
}
...
foreach outcome in y1_left y2_right {
    ...
    ereturn clear
    capture noisily `model' from(nb2rs_start) startvalues(fixedonly, iterate(15)) iterate(0) log
    local eval_rc = _rc
    ...
    ereturn clear
    capture noisily `model' from(nb2rs_start) startvalues(fixedonly, iterate(15)) iterate(15) log showtolerance
    local fit_rc = _rc
    ...
    if e(converged) != 1 | e(N) != `expected_n' | missing(e(ll)) | abs(e(ll)-scalar(nb2rs_r_`outcome'_ll)) > 1e-4 | scalar(nb2rs_ok) != 1 {
...
* covariances are fixed at zero. The two equations use disjoint id groups.
* Stata uses the 100-group union correction; R uses 50 groups per component.
* The returned covariance audit will align these corrections explicitly.
local model "gsem (y1_left <- x z I1[id]@1 c.x#S1[id]@1, family(nbinomial mean) link(log)) (y2_right <- x z I2[id]@1 c.x#S2[id]@1, family(nbinomial mean) link(log)), cov(I1[id]*S1[id] I2[id]*S2[id] I1[id]*I2[id]@0 I1[id]*S2[id]@0 S1[id]*I2[id]@0 S1[id]*S2[id]@0) intmethod(laplace) adaptopts(tolerance(1e-12)) vce(robust)"
di as txt "=== JOINT PARAMETER TEMPLATE (NO ESTIMATION) ==="
`model' noestimate startvalues(zero)
matrix nb2rs_start = e(b)
mata:
nb2rs_s = st_matrixcolstripe("nb2rs_start")
nb2rs_w = ("y1_left", "x" \ "y1_left", "z" \ "y1_left", "_cons" \ "/y1_left", "lnalpha" \ ///
    "/", "var(S1[id])" \ "/", "var(I1[id])" \ "/", "cov(I1[id],S1[id])" \ ///
    "y2_right", "x" \ "y2_right", "z" \ "y2_right", "_cons" \ "/y2_right", "lnalpha" \ ///
...
di as result "NB2_RS_DISJOINT_COMPLETE=1"
