# Run here after parsing the returned partial log; optional comparison CSV.
...
rj <- r$systems$partial_higher; rj$logLik <- sum(vapply(r$components[c("y1_partial", "y2_partial")], `[[`, numeric(1), "logLik"))
comparison <- rbind(comparison, compare(f$joint, rj, "partial_joint_raw"))
