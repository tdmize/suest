...
result <- setNames(lapply(c("Stata", "R"), function(point) {
  ...
  audit_nbinom2_rs_joint(f$joint, r$systems$partial_higher, f$data,
    c("y1_partial", "y2_partial"), "higher", f$components, evaluate_at = point)
}), c("Stata", "R"))
for (i in 1:2) {
  y <- c("y1_partial", "y2_partial")[i]; B <- result$R$components[[i]]$bread
...
saveRDS(result, "../../tests/testthat/fixtures/glmmtmb-nbinom2-rs-partial-audit.rds")
cat("NB2_RS_PARTIAL_INDEPENDENT_AUDIT_COMPLETE=1\n")
