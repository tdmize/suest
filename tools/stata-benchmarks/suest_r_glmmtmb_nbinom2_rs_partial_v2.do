...
log using suest_r_glmmtmb_nbinom2_rs_partial_v2.log, replace text name(nb2rs)
di as result "BENCHMARK_FILE_REVISION=2"
di as txt "NB2 RANDOM SLOPES: PARTIAL OVERLAP, 20 HIGHER CLUSTERS; THREE FITS ONLY"
...
do suest_r_glmmtmb_nbinom2_rs_partial_starts.do
matrix nb2rs_components = J(2, 7, .)
matrix rownames nb2rs_components = y1_partial y2_partial
...
foreach outcome in y1_partial y2_partial {
    local ++row
    local expected_n = cond("`outcome'" == "y1_partial", 975, 980)
...
local model "gsem (y1_partial <- x z I1[id]@1 c.x#S1[id]@1, family(nbinomial mean) link(log)) (y2_partial <- x z I2[id]@1 c.x#S2[id]@1, family(nbinomial mean) link(log)), cov(I1[id]*S1[id] I2[id]*S2[id] I1[id]*I2[id]@0 I1[id]*S2[id]@0 S1[id]*I2[id]@0 S1[id]*S2[id]@0) intmethod(laplace) adaptopts(tolerance(1e-12)) vce(cluster higher)"
di as txt "=== JOINT PARAMETER TEMPLATE (NO ESTIMATION) ==="
`model' noestimate startvalues(zero)
matrix nb2rs_start = e(b)
mata:
nb2rs_s = st_matrixcolstripe("nb2rs_start")
nb2rs_w = ("y1_partial", "x" \ "y1_partial", "z" \ "y1_partial", "_cons" \ "/y1_partial", "lnalpha" \ ///
    "/", "var(S1[id])" \ "/", "var(I1[id])" \ "/", "cov(I1[id],S1[id])" \ ///
    "y2_partial", "x" \ "y2_partial", "z" \ "y2_partial", "_cons" \ "/y2_partial", "lnalpha" \ ///
...
if e(converged) != 1 | e(N) != 1000 | e(N_clust) != 20 | missing(e(ll)) | abs(e(ll)-scalar(nb2rs_target_ll)) > 1e-4 | scalar(nb2rs_ok) != 1 {
    di as error "JOINT_CHECK_FAILED: return this log."
    log close nb2rs
    exit 459
}
di as result "NB2_RS_PARTIAL_HIGHER_COMPLETE=1"
