...
log using suest_r_panel_probit_re_crosslang_benchmark.log, replace text name(panelreprobit)

di as result "BENCHMARK_FILE_REVISION=1"
di as txt "SUEST R RANDOM-EFFECTS PANEL PROBIT CROSS-LANGUAGE BENCHMARK"
di as txt "Stata version: `c(stata_version)'"
which suest2

import delimited using suest_r_panel_probit_re_crosslang_benchmark.csv, clear varnames(1)
xtset id time

di as txt "=== CASE 1A: BALANCED MODEL Y1, NONADAPTIVE 12-POINT QUADRATURE ==="
quietly xtprobit y1 x z, re intmethod(ghermite) intpoints(12)
...
quietly xtprobit y2 x z, re intmethod(ghermite) intpoints(12)
di as txt "N=" e(N) " N_g=" e(N_g) " LL=" %21.15g e(ll)
matrix list e(b), format(%21.15g)
matrix list e(V), format(%21.15g)
...
quietly xtprobit y1 x z, re intmethod(mvaghermite) intpoints(12)
di as txt "Y1_ADAPTIVE_N=" e(N) " N_g=" e(N_g) " LL=" %21.15g e(ll)
matrix list e(b), format(%21.15g)
matrix list e(V), format(%21.15g)
estimates store y1_balanced

quietly xtprobit y2 x z, re intmethod(mvaghermite) intpoints(12)
...
quietly xtprobit y1_partial x z, re intmethod(mvaghermite) intpoints(12)
estimates store y1_partial
quietly xtprobit y2_partial x z, re intmethod(mvaghermite) intpoints(12)
...
quietly xtprobit y1_left x z, re intmethod(mvaghermite) intpoints(12)
estimates store y1_left
quietly xtprobit y2_right x z, re intmethod(mvaghermite) intpoints(12)
...
di as result "PANEL_PROBIT_RE_CROSSLANG_BENCHMARK_COMPLETE=1"
log close panelreprobit
