...
fit_glmm_poisson <- function(formula, data = dat) glmmTMB::glmmTMB(
  formula, data = data, family = poisson("log"))
...
y1 <- fit_glmm_poisson(y1 ~ x + z + (1 | id))
y2 <- fit_glmm_poisson(y2 ~ x + z + (1 | id))
...
y1_partial <- fit_glmm_poisson(y1_partial ~ x + z + (1 | id))
y2_partial <- fit_glmm_poisson(y2_partial ~ x + z + (1 | id))
...
y1_left <- fit_glmm_poisson(y1_left ~ x + z + (1 | id), left_data)
y2_right <- fit_glmm_poisson(y2_right ~ x + z + (1 | id), right_data)
...
saveRDS(reference, "glmmtmb_poisson_ri_r_reference.rds")
cat("GLMMTMB_POISSON_RI_R_REFERENCE_COMPLETE=1\n")
