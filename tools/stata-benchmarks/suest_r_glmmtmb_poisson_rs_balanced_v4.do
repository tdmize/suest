...
log using suest_r_glmmtmb_poisson_rs_balanced_v4.log, replace text name(glmmrs)
di as result "BENCHMARK_FILE_REVISION=4"
di as txt "BALANCED JOINT MODEL ONLY; TIGHTER INTERNAL INTEGRATION TOLERANCE"
di as txt "Only numerical change from v3: adaptopts(tolerance(1e-12)); original default was 1e-8."
di as txt "Outer convergence tolerances are unchanged. Maximum 15 optimization iterations."
...
local model "gsem (y1 <- x z I1[id]@1 c.x#S1[id]@1, family(poisson) link(log)) (y2 <- x z I2[id]@1 c.x#S2[id]@1, family(poisson) link(log)), cov(I1[id]*S1[id] I2[id]*S2[id] I1[id]*I2[id]@0 I1[id]*S2[id]@0 S1[id]*I2[id]@0 S1[id]*S2[id]@0) intmethod(laplace) adaptopts(tolerance(1e-12)) vce(robust)"
...
if `ok' {
    di as result "POISSON_RS_BALANCED_PRECISION_COMPLETE=1"
}
else {
    di as error "POISSON_RS_BALANCED_PRECISION_INCOMPLETE=1"
}
