...
stopifnot("BENCHMARK_FILE_REVISION=1" %in% lines, "LOGIT_RS_DISJOINT_COMPLETE=1" %in% lines,
  all(paste0("LOGIT_RS_COMPONENT_", c("y1_left", "y2_right"), "_COMPLETE=1") %in% lines))
...
stopifnot(length(gaps) == 3L, length(changes) == 3L, all(is.finite(gaps)),
  ...
  sum(lines == "COMPONENT_FIT_RC=0") == 2L, "DISJOINT_FIT_RC=0" %in% lines,
...
components <- setNames(lapply(c("y1_left", "y2_right"), function(y) {
  ...
  stopifnot(result$nobs == 600L, result$n_groups == 50L)
  result
}), c("y1_left", "y2_right"))
start <- which(lines == "=== DISJOINT JOINT GSEM LAPLACE (MAXIMUM 15 ITERATIONS) ===")
stopifnot(length(start) == 1L)
joint <- logitrs_joint(lines[start:length(lines)], c("y1_left", "y2_right"), 1200, 100)
...
saveRDS(fixture, if (length(args) > 1L) args[2L] else "../../tests/testthat/fixtures/glmmtmb-logit-rs-stata-disjoint-v1.rds")
cat("LOGIT_RS_DISJOINT_FIXTURE_COMPLETE=1\n")
